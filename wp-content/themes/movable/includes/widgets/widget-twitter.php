...
class TJ_Twitter_Widget extends WP_Widget {

	function TJ_Twitter_Widget() {
		$widget_ops = array('classname' => 'widget_twitter', 'description' => __( "Display your tweets from Twitter") );
		$this->WP_Widget('twitter', __('ThemeJunkie - Twitter'), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		if ( !$tweets = wp_cache_get( 'widget-twitter-' . $this->number , 'widget' ) ) {
			...
			$response = wp_remote_get( $twitter_json_url, array( 'User-Agent' => 'TJ Twitter Widget' ) );
		...
		if ( 'error' != $tweets ) :
			...
			foreach ( (array) $tweets as $tweet ) {
				...
				echo "<li>{$before_tweet}{$text}{$before_timesince}<span class='timesince'>" . str_replace(' ', '&nbsp;', wpcom_time_since(strtotime($created_at))) . "&nbsp;ago</span></li>\n";
				$tweets_out++;
			}
			...
			if ( 401 == wp_cache_get( 'widget-twitter-response-code-' . $this->number , 'widget' ) )
				echo "<div>" . __("Error: Please make sure the Twitter account is <a href='http://help.twitter.com/forums/10711/entries/14016'>public</a>.") . "</div>";
			else
				echo "<div>" . __("Error: Twitter did not respond. Please wait a few minutes and refresh this page.") . "</div>";
		endif;
...
add_action( 'widgets_init', 'tj_twitter_widget_init' );
function tj_twitter_widget_init() {
	register_widget('TJ_Twitter_Widget');
}
